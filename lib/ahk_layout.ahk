...
class AhkLayout {
    ...
    static FillLeaf(arr, dataPtr, count, itemSize, typeCode, layout := 0) {
        ...
        ; Validate the variant layout BEFORE writing anything: the zero tail
        ; starts at symbolOffset+4, so a symbol below the value field would
        ; wipe the value it just wrote.
        if layout["valueOffset"] + 8 > layout["variantSize"]
            or layout["symbolOffset"] + 4 > layout["variantSize"]
            or layout["symbolOffset"] <= layout["valueOffset"]
            throw Error("AhkLayout invalid variant offsets: value="
                . layout["valueOffset"] " symbol=" layout["symbolOffset"]
                . " size=" layout["variantSize"])
        cap := arr.Capacity
        ; This is the overflow guard the native filler cannot perform: the
        ; item buffer only holds Capacity variants.  Checking after the
        ; writes would be pointless because the capacity field is written
        ; as part of the fill.
        if count > cap
            throw Error("AhkLayout.FillLeaf count " count
                . " exceeds array capacity " cap)
        obj := ObjPtr(arr)
