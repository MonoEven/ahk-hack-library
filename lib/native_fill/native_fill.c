...
__declspec(dllexport) int cnp_fill_leaf(
    ...
    u32 m_length_off, u32 m_capacity_off, u32 capacity,
    ...
    u32 sym_int, u32 sym_float)
{
    ...
    if (count > 0 && data == 0)
        return 1;
    /* The item buffer holds at most `capacity` variants; refuse anything
     * larger instead of overflowing the allocation. */
    if ((u64)count > capacity)
        return 4;
    if (count > 0xFFFFFFFFull)
        return 5; /* mLength/mCapacity are 32-bit */
